...
with st.container():
    ...
    with st.expander("🟣  **Situational Behaviors**  ", expanded=False):
        ...
        for i in situational_df.index:
            ...
            if f"situational_down_btn_{i}" not in st.session_state:
                st.session_state[f"situational_down_btn_{i}"] = False
            if f"situational_up_btn_{i}" not in st.session_state:
                st.session_state[f"situational_up_btn_{i}"] = False
